...
- **TypeScript is deliberately held at `~6.0.3` — do not bump to 7.x yet.** TS 7.0 (the native Go port) installs cleanly and `vp check` still passes, because tsgolint typechecks independently of the TypeScript API — but **`vp pack` fails**: tsdown 0.22.13 warns `TypeScript 7.0 does not yet have a stable API and is experimental` and then aborts with `tsgo did not generate dts file for … src/preset-full.ts`. `vp pack` is also `prepublishOnly`, so this breaks releases, not just local builds. Re-test only once a Vite+ release bundles a tsdown whose `rolldown-plugin-dts` supports the TS 7 API — and gate the bump on `vp pack`, since a green `vp check` proves nothing here.
