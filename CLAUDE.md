...
### Vite+ 0.2.x toolchain (since #458; aligned to 0.2.7)

Vite+ 0.2.7 bundles **Vite 8.1.5 + Rolldown 1.2.0 + Vitest 4.1.10 + Oxfmt 0.60.0 + Oxlint 1.75.0 + oxlint-tsgolint 7.0.2001 + tsdown 0.22.14** inside the `vite-plus` toolchain (run `vp --version` for the current list — the bundled pins move with every Vite+ release). The separate `@voidzero-dev/vite-plus-test` package is **dead** (no 0.2.x exists). Consequences for dep management:
...
- **TypeScript is deliberately held at `~6.0.3` — do not bump to 7.x yet.** TS 7.0 (the native Go port) installs cleanly and `vp check` still passes, because tsgolint typechecks independently of the TypeScript API — but **`vp pack` fails**: tsdown warns `TypeScript 7.0 does not yet have a stable API and is experimental` and then aborts with `tsgo did not generate dts file for … src/preset-full.ts`. `vp pack` is also `prepublishOnly`, so this breaks releases, not just local builds. Re-tested and still failing identically on tsdown 0.22.14 (Vite+ 0.2.7); first seen on 0.22.13. Re-test only once a Vite+ release bundles a tsdown whose `rolldown-plugin-dts` supports the TS 7 API — and gate the bump on `vp pack`, since a green `vp check` proves nothing here.
