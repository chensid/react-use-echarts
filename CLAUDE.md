...
### Vite+ 0.3.x toolchain (since #458; aligned to 0.3.0)

Vite+ 0.3.0 bundles the whole toolchain — Vite, Rolldown, Vitest, Oxfmt, Oxlint, oxlint-tsgolint, tsdown — inside the `vite-plus` package (run `vp toolchain` for the exact bundled pins; they move with every Vite+ release). The separate `@voidzero-dev/vite-plus-test` package is **dead** (no 0.2.x exists). Consequences for dep management:
...
- **TypeScript 7 (`~7.0.2`, the native Go port) is adopted** (#525). `vp check` was never at risk (tsgolint typechecks independently of the TypeScript API); `vp pack` works because every `pack[]` entry in `vite.config.ts` pairs `tsconfig: "tsconfig.lib.json"` with `dts: { tsgo: true }` — TS 7's native declaration compiler consumes a single leaf project config, not the solution root. Keep those two settings paired on all three entries: reverting to the old `dts: { build: true }` path (rolldown-plugin-dts via the TS API) aborts with `tsgo did not generate dts file for … src/preset-full.ts`. `vp pack` is also `prepublishOnly`, so gate any dts/tsconfig change on a green `vp pack` — a green `vp check` proves nothing here.
...
- React Compiler enabled via `@vitejs/plugin-react` + `@rolldown/plugin-babel` (`reactCompilerPreset()`), wired in three places in `vite.config.ts`: the top-level `plugins` (dev/build) and two of the `pack[]` entries (the library build — this is the one that produces the `c(N)` memo caches in `dist/`). **TODO (native, Babel-free path), re-surveyed 2026-08-26:**
  - The Babel implementation is now a legacy branch. `babel-plugin-react-compiler` has published nothing to npm since 2026-05-08 and `latest` (1.0.0) dates from 2025-10-07; upstream React moved compiler development to Rust (`[compiler] Port React Compiler to Rust`, react/react#36173, 2026-06-09) and dropped its own OXC/SWC plugins two days later "in favor of them being handled by those projects". Bugs we hit in 1.0.0 will not be fixed there — see the destructuring-defaults bail documented on `useEcharts`.
  - **The app-side gate is CLEARED.** `@vitejs/plugin-react` 6.1.0 exposes `compiler?: boolean | ReactCompilerOptions` (`@experimental`, needs `oxc-transform-react`), and `@rolldown/plugin-babel` / `babel-plugin-react-compiler` / `oxc-transform-react` are all optional peers, so the Babel trio can be dropped from the Vite pipeline. This repo already resolves plugin-react 6.1.0.
  - **The remaining blocker is `pack[]`.** `react({ compiler: true })` is a Vite plugin and does not reach the tsdown/rolldown library build. `vite-plus`'s pack config exposes no React Compiler option, and no official rolldown-side plugin exists (`unplugin-react-compiler`, `rolldown-plugin-react-compiler`, `@oxc-project/rolldown-plugin-react` are all unpublished). Switching only the app side is worthless: the showcase is not published, and `@babel/core` would still be required by `pack[]`.
  - Verified against this repo's own source with `oxc-transform-react@0.147.0`: it compiles every file the Babel plugin compiles, plus `useEcharts` (`_c(14)`), emits near-identical output for `EChart` (`_c(40)` vs Babel's `c(41)`), and bails identically on eslint-suppressed hooks. The port works; only the packaging is missing.
  - **Re-evaluate when** `vite-plus`/tsdown exposes the compiler for `pack[]`, or an official rolldown plugin ships. At that point the switch is a config change rather than a hand-written plugin. Watch the Vite+ changelog, the `@vitejs/plugin-react` changelog, and oxc-project/oxc#26013 (React-parity TODO tracker). Note that React's own `crates/TODO.md` parity table is NOT a usable signal — it has one commit, from the day the port landed.
