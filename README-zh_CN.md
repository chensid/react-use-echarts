...
| 错误路由    | `onError` 统一处理 init / setOption / dispatch | 手动 try/catch         |
...
可运行示例参见 [`examples/core-entry/CoreEntryChart.tsx`](./examples/core-entry/CoreEntryChart.tsx)。
...
- **不要忘记注册 ECharts 模块** — `useEcharts()` 在 ECharts 全局 registry 上初始化实例，所以图表/组件/渲染器/特性必须先注册（通过 `registerEchartsFull()` 或 `echarts.use([...])`）。忘记注册通常表现为 `Renderer 'undefined' is not imported` 报错，或图表静默不渲染；参见 [注册 ECharts 模块](#注册-echarts-模块)。
...
> `react-use-echarts/core` 自 v2.1 起是默认入口的别名（两者均为 modular），将在 v4 移除。请把 `from "react-use-echarts/core"` 迁移到 `from "react-use-echarts"`。
...
v2.1 不再从默认入口副作用 import `"echarts"` —— 库已完全 modular，与 `vue-echarts` / `nuxt-echarts` / `react-chartjs-2` 立场一致。Hook / 组件 API 不变，只需在应用入口**加一行**：
...
`react-use-echarts/core` 子入口自 v2.1 起已废弃，行为与默认入口完全一致（均为 modular）。原有 `from "react-use-echarts/core"` 仍可工作，但将在 v4 移除，请择机迁移到默认入口。
