...
- bacc088: Raise the declared Node.js tooling floor on 22.x from 22.18 to 22.19, matching the `size-limit` 14 requirement introduced with the latest development dependency refresh: Node.js 22.19+ on 22.x, 24.11+ on 24.x, or 26+. The CI job that guards the 22.x minimum now pins 22.19.0 so the tested floor and the declared floor stay identical.
...
- 2a388bc: Upgrade Vite+ toolchain to 0.2.6 (Vite 8.1.5, Rolldown 1.2.0, tsdown 0.22.13, Oxlint 1.75.0, Oxfmt 0.60.0) and bump dev dependencies (@vitejs/plugin-react 6.0.4, happy-dom 20.11.1, publint 0.3.22). No API changes — dist is rebuilt on the updated toolchain.
